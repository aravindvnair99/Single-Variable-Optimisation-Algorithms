#6 Successive Quadratic Estimation Method
from math import exp, sin, cos, log
print("Please give termination parameter as zero to proceed with number of iterations.")
x=list()
x1,d,e=map(float,input("Enter initial point, delta and termination parameter: ").split())
...
while(True):
  ...
  newf=list()
  for i in range(3):
    newx.append(xlist[flist.index(sortlist[i])])#flist.index(sortlist[i]) returns index of corresponding f element in original list
  newx=sorted(newx)
  for i in range(3):
    newf.append(flist[xlist.index(newx[i])]) # xlist.index(newx[i]) returns index of corresponding x element in original list
  x1,x2,x3=newx
  f1,f2,f3=newf
  ...
  if((abs(Fmin-fxbar)<e and abs(xmin-xbar)<e)or c>=j):
    break
print("Point corresponding to x=",round(newmin,5)," is the minimum of the function.")
